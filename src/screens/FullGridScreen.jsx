  ...
  return (
    <div className="screen" style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
      ...
      <div style={{ padding: '12px 16px 8px' }}>
        <h2 style={{
          margin: 0, fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 22,
        ...
        <div style={{
          ...
          {counts.miss > 0
            ? t(lang, 'missingCount', counts.miss)
            : t(lang, 'albumComplete')}
        </div>
      </div>
      ...
      {/* Grid */}
      <div style={{
        margin: '0 var(--screen-margin)', flex: 1, paddingBottom: 16,
        display: 'grid',
        ...
        {filtered.map(s => {
          const owned = s.qty >= 1;
          const accent = getTeamAccent(s.team.code);
          return (
            <button
              key={s.id}
              onClick={() => setSticker(s.id, owned ? Math.max(0, s.qty - 1) : 1)}
              title={`${s.team.code} ${s.num} — x${s.qty}`}
              style={{
